...
func (l *GroupLoader) Load(key string) ([]*model.Group, error) {
...
func (l *GroupLoader) LoadThunk(key string) func() ([]*model.Group, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() ([]*model.Group, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *GroupLoader) LoadAll(keys []string) ([][]*model.Group, []error) {
	results := make([]func() ([]*model.Group, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	groups := make([][]*model.Group, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *GroupLoader) LoadAllThunk(keys []string) func() ([][]*model.Group, []error) {
	results := make([]func() ([]*model.Group, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *GroupLoader) Prime(key string, value []*model.Group) bool {
	l.mu.Lock()
	var found bool
	if _, found = l.cache[key]; !found {
		// make a copy when writing to the cache, its easy to pass a pointer in from a loop var
		// and end up with the whole cache pointing to the same value.
		cpy := make([]*model.Group, len(value))
		copy(cpy, value)
		l.unsafeSet(key, cpy)
	}
	l.mu.Unlock()
...
func (l *GroupLoader) Clear(key string) {
	l.mu.Lock()
	delete(l.cache, key)
	l.mu.Unlock()
}

func (l *GroupLoader) unsafeSet(key string, value []*model.Group) {
	if l.cache == nil {
		l.cache = map[string][]*model.Group{}
	}
	l.cache[key] = value
}
...
func (b *groupLoaderBatch) keyIndex(l *GroupLoader, key string) int {
